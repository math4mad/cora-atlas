# REFS · 迭代 II 的文献弹药（Crossref 逐条核验 2026-09-20；未核者标 †）

## 「人人等速」判决句的支撑
- Bransford & Johnson (1972). Contextual prerequisites for understanding. *J. Verbal Learning & Verbal Behavior*. DOI:10.1016/S0022-5371(72)80006-9 —— 有图式者回忆量碾压，仓库假说的直接死刑
- Bartlett (1932) *Remembering* † —— 图式吸收新知的古典
## 动作技能的基底（自行车）
- Schmidt (1975). A schema theory of discrete motor skill learning. *Psychological Review*. DOI:10.1037/h0076770 —— 技能=关系不变式而非轨迹库存
- 相关综述近作：Barreiros et al., A schema theory of motor learning（Handbook 章, DOI:10.4324/9781003542391-15）
## 类别/原型（空间感→车的契合度梯度）
- Rosch (1975). Cognitive representations of semantic categories. *JEP:General*. DOI:10.1037/0096-3445.104.3.192
## 专家知觉 = 基底检索（旁证）
- Chase & Simon (1973). Perception in chess. *Cognitive Psychology*. DOI:10.1016/0010-0285(73)90004-2
## 加工层级（编码相对于既有结构）
- Craik & Lockhart (1972) Levels of processing †（DOI 以 2014 再版卷册核验：10.4324/9781315796192）
## 海马绑定/皮层基底（理论骨架的神经版）
- McClelland, McNaughton & O'Reilly (1995). Why there are complementary learning systems. *PLoS Biol* 版近续：Considerations arising from CLS. *Hippocampus*. DOI:10.1002/(SICI)1098-1063(1996)6:6<654::AID-HIPO8>3.0.CO;2-G
## 容量天花板（无基底之物的宿命）
- Miller (1956). The magical number seven, plus or minus two.（再版章核验：10.1525/9780520318267-011）
## 延迟学习（不贴图式者）
- Bruner. A theory of the delay of learning †（The Cognitive Revolution 辑内核验：10.5040/9781472541369.ch-008）
## LLM 侧
- von Oswald et al. (2023). Transformers learn in-context by gradient descent. ICLR †（arXiv:2212.07674）
- Huth et al. (2016). Natural language selects for a shared semantics map in the brain. *Nature* †（DOI 10.1038/nature.2016.19786）
## 园区实证（pilot evidence 引用口径）
- 探针落错空间现场记录：iterations/2026-09-20-missed-catch-maigui.md
- xdom 50 卷（同 token 异空间塌缩的组卷设计）：orig/xdom-probes-brief.docx；运行时结果候 Kaggle 波次

## 父本升格：Gärdenfors《Conceptual Spaces: The Geometry of Thought》(MIT Press 2000)
> 2026-09-22 交叉论文查询验尸：外部 AI 六条交集分析所引原书句，逐页对 z-lib 扫描本 (sha256 d0d86c3d…) 核验，PDF 页码与印刷页码零偏移。
- p10 (Introduction) —— "Concept learning is closely tied to the notion of similarity, which has turned out to be problematic for the symbolic and associationistic approaches. Here, I advocate a third form… geometrical structures" ✅ 逐字真（曾被问答机误判为 Bishop 书中无 → 冤案平反）；**我园两篇论文=他所寻"第三层"的计算实现，此句可作题记**
- p52 (Ch2) —— 三层表征 symbolic/conceptual/associationist(subconceptual) ✅
- p136 (Ch4 §4.3.2) —— "similarity and distances in conceptual spaces are intimately connected" ✅；同页"similarity, in turn, can be used to define concepts"（问答机引作 "intimately connected with the notion of a concept"，⚠️ 缝合句，意义近而措辞非原话）
- p166 (Ch4 §4.9) —— 广义 Voronoi / prototypical area ✅（问答机作 p165 且措辞略走样）
- p278 (Ch7) —— Kohonen SOM "…very much like it would be represented in a conceptual space" ✅ —— 我们 A4 流形公理的先声
- p293 (Ch7) —— 三层表征=不同分辨率尺度（高维向量→低维结构向量）✅ —— 与 LoRA 低秩 ΔW 直接对接
- **⚠️ 2026-09-22 夜谱系改判（主人亲口）**：我园论文与 G 书非继承关系，是**独立收敛**——真祖源 = Chang & Tsao (Cell 2017, 面孔 50 调谐轴) + "认知即 JPEG：内置基、记忆只存系数"直觉 (2017→)。G 书降为**并行祖源**（"书在架上未读完，开篇或有渗漏"）。Letter 032 叙事随之改 v2"请判读收敛是真是巧"。见 iterations/2026-09-22-lineage-reversal-and-structure-is-intelligence.md
